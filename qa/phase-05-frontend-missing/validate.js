#!/usr/bin/env node
/*
 * Phase-05 frontend "what is missing?" tab validation. Fully autonomous.
 *
 ...
 *   4.  Run `npx playwright test` (chromium) — tests/missing-tab.spec.js drives
 *       one ordered walk: load -> click #main-btn (start) -> wait listening ->
 *       node ../test-app/index.js --scenario phase-05 --endpoint http://localhost:4318 ->
 *       wait ~3s -> click stop -> wait "report ready" -> switch to the
 *       "what is missing?" tab -> 10 phase-5 assertions.
 ...
 *
 * A failure to run the session walk itself (start/stop/report-ready/tab switch)
 * or to run Playwright at all (missing browser, config error, no results file)
 * is reported as assertion 99 — a QA-infrastructure failure, NOT a phase
 * assertion failure.
 */
...
async function main() {
  ...
  try {
    ...
    const pw = spawnSync('npx', ['playwright', 'test'], {
      ...
      env: {
        ...process.env,
        PHASE5_BASE_URL: FRONTEND,
        PHASE5_OTLP_ENDPOINT: 'http://localhost:4318',
    ...
    // -- 5a. the session walk (start/stop/report-ready/tab switch) is infra,
    //        not a phase assertion — check its sentinel first.
    let walk = null;
    ...
    log(`  backend report.missing count=${walk.rawMissingCount}; expected cards=${JSON.stringify(walk.expected)}`);
