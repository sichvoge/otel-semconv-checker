#!/usr/bin/env node
/*
 * Phase-04 frontend "what is broken?" tab validation. Fully autonomous.
 *
 ...
 *   4.  Run `npx playwright test` (chromium) — tests/broken-tab.spec.js drives
 *       one ordered walk: load -> click #main-btn (start) -> wait listening ->
 *       node ../test-app/index.js --scenario phase-04 --endpoint http://localhost:4318 ->
 *       wait ~3s -> click stop -> wait "report ready" -> 11 phase-4 assertions on
 *       the report view.
 *   5.  Parse test-results/walk-status.json + test-results/results.json into a
 *       per-assertion summary
 *   6.  COMPOSE_PROFILES=app docker compose ... down -v --remove-orphans
 *   7.  Print summary, exit 0 (all 11 pass) / 1 (any fail or infra error)
 *
 * Only reads/writes under qa/. Never mutates implementation files.
 *
 * A failure to run the session walk itself (start/stop/report-ready) or to run
 * Playwright at all (missing browser, config error, no results file) is reported
 * as assertion 99 — a QA-infrastructure failure, NOT a phase assertion failure.
 */
...
async function main() {
  ...
  try {
    ...
    const pw = spawnSync('npx', ['playwright', 'test'], {
      ...
      env: {
        ...process.env,
        PHASE4_BASE_URL: FRONTEND,
        PHASE4_OTLP_ENDPOINT: 'http://localhost:4318',
    ...
    // -- 5a. the session walk (start/stop/report-ready) is infra, not a phase
    //        assertion — check its sentinel first.
    let walk = null;
    ...
    if (!walk || walk.ok !== true) {
      infraFailure = `session walk did not reach the report view: ${
        walk && walk.error ? walk.error.split('\n')[0] : 'walk-status.json missing'
      }`;
      throw new Error(infraFailure);
    }
    log('Session walk reached the report view (idle -> listening -> report).');

    // -- 5b. parse phase-assertion results -------------------------------
    ...
    if (infraFailure) {
      assert(99, `QA-infra failure: ${infraFailure}`, false, 'validate.js / Playwright / session walk could not run — not a phase failure');
